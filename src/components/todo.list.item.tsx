...
export const TodoListItem: React.FC<TodoListItemProps> = (props) => {
    ...
    const setRecord = () => {
        ...
        window.location.replace(`#/select/${todo.id}`)
    }
    ...
    return (
        <Alert show={show} onClose={() => handleClose()} variant={"info"} dismissible={!props.isSelected}>
            ...
            <div className="d-flex justify-content-end" style={{ marginTop: "15px" }}>
                ...
                {props.isSelected ?
                    <Button onClick={() => {dispatch({ type: ActionType.CLEAR });window.location.replace(`#/`)} } variant="danger" style={{ marginRight: "8px" }}>
                        Back
                </Button> :
                    <Button onClick={() => setRecord()} variant="info">
                        Select
            </Button>
                }
